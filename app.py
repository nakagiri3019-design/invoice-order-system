...
def consult_ai(
    ...
    system_prompt = (
        ...
        "日本語で回答してください。\n\n"
        "【DOCUMENT_DATAのルール】\n"
        "- 確定した情報だけをフィールドに入れる。未確認の情報は空文字またはデフォルト値のままにする。\n"
        "- clientは画像・ユーザー入力から確定した会社名だけを入れる。不明な場合は空文字。\n"
        "- itemsは品目名が確定してから入れる。未確定の場合はitemsを空リスト[]にする。\n"
        "- unit_priceは画像・ユーザー入力から読み取れた金額だけを入れる。不明な場合は0のまま。\n"
        "- 振込先（bank）情報はDOCUMENT_DATAに含めない。発行元プロフィールから自動で使われる。\n"
        "- itemsのunit_priceは画像から読み取った金額を必ず入れる。0のままにしない。\n"
        ...
        "2. 不足している項目は一度に全部聞かない。最も重要な1項目だけを質問する。\n"
        "3. 質問の優先順位：\n"
        "   1位：品目名（これがないとPDFが作れない）\n"
        "   2位：支払期日（宛先・金額が決まってから）\n"
        "   3位：書類番号（任意なので後回し）\n"
        "4. ユーザーが「〇〇の請求書を作って」と言った場合、既に分かっている情報をまとめて提示し、不足している情報だけを1つ質問する。\n"
        "5. 通帳・領収書・請求書などの画像が添付された場合：\n"
        "   - まず画像から読み取れた情報をまとめて提示する\n"
        "   - 不足している項目のうち最重要の1つだけを質問する\n"
        "   - 「以下の情報が確認できました。品目名だけ教えてください。」という形式にする\n"
        "6. DOCUMENT_DATAのJSONは必ず返答の末尾に含める。\n"
        "   分かる項目は埋める。分からない項目はmissing_fieldsに入れる。\n"
        "   missing_fieldsには一度に1〜2項目だけ入れる。\n\n"
        "【画像・通帳読み取り時の特別ルール】\n"
        "- 通帳・振込明細の画像では、日付・振込先名・金額が読み取れる場合が多い。解像度が低くても読める情報は積極的に使う。\n"
        "- 「カ）インタラクト」は「株式会社インタラクト」として扱う。\n"
        "- 「振込」行の金額は請求金額として使う。数量1式・単価=その金額として仮設定する。\n"
        "- ユーザーが「5月末入金」と言った場合、支払期日は「2026年5月31日」と推定する。\n"
        "- 振込先情報（〇〇銀行など）は、登録済みプロフィールの振込先を使うため、画像から推測して表示しない。\n"
        "- 通帳から読み取れた金額は必ずunit_priceに設定する。品目名が未確定でも金額はitemsに入れる。\n"
        "- 金額が確定している場合、数量・単価を再度ユーザーに聞かない。数量=1、単位=式、単価=読み取り金額で固定する。\n"
        "- 質問は一度に必ず1つだけ。金額が判明している場合は品目名だけを聞く。\n"
        "- 読み取れた情報は再確認しない。品目名だけを1つ質問する。\n"
        "- 金額が判明している場合、DOCUMENT_DATAのitemsには以下の形式で入れる：\n"
        '  items: [{"name": "", "qty": 1, "unit": "式", "unit_price": 読み取った金額}]\n'
        '  品目名は空文字のままで、missing_fieldsに"items"を入れる。\n'
        "- 理想の返答形式：\n"
        "  「通帳画像から、{会社名}から{日付}に{金額}円の入金があることを確認しました。\n"
        "   請求書の支払期日は{支払期日}として設定します。\n"
        "   品目名だけ教えてください。例：業務委託料、コンサルティング費など」\n\n"
        "【絶対禁止のダミー値】\n"
        "以下の値は絶対にDOCUMENT_DATAに入れない：\n"
        '- client: "株式会社〇〇" または空文字以外の推測値\n'
        '- items name: "AI開発支援業務"、"業務委託料"など、ユーザーが言っていない品目名\n'
        "- unit_price: 1000000（ユーザーも画像も指定していない場合）\n"
        '- bank情報（振込先）：登録済みプロフィールの振込先情報を使うため、画像から推測した振込先は絶対に表示しない。\n'
        '  振込先が未登録の場合は「振込先未登録」または空欄にする。\n'
        '  「〇〇銀行」「〇〇支店」「0000000」などのダミー振込先は絶対禁止。\n'
        "- 画像・ユーザー入力から確認できた情報だけを入れる。"
    )
    if current_data:
